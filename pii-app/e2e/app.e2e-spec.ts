...
describe('Starting tests for pii-app', function() {
  ...
  it('website title should be pii-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('pii-app');
    })
  });

  it('network-name should be pii-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('pii-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be pii-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('pii-app');
    });
  });
