 ...
 * @param {org.brighteyed.piinetwork.AuthorizeAccess} authorize - the authorize to be processed
 * @transaction
 */
async function authorizeAccess(authorize) {  // eslint-disable-line no-unused-vars
    ...
    if(index < 0) {
        ...
        // emit an event
        const event = getFactory().newEvent('org.brighteyed.piinetwork', 'MemberEvent');
        event.memberTransaction = authorize;
        emit(event);

        // persist the state of the member
        const memberRegistry = await getParticipantRegistry('org.brighteyed.piinetwork.Member');
 ...
 * @param {org.brighteyed.piinetwork.RevokeAccess} revoke - the RevokeAccess to be processed
 * @transaction
 */
