...
describe('Starting tests for brighteyed-app', function() {
  ...
  it('website title should be brighteyed-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('brighteyed-app');
    })
  });

  it('network-name should be brighteyed-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('brighteyed-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be brighteyed-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('brighteyed-app');
    });
  });
    ...
    it('AddRecordTransaction component should be loadable',() => {
      page.navigateTo('/AddRecordTransaction');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('AddRecordTransaction');
      });
    });
